...
double computeFreq(vector<vector<Int>>& rank, vector<Int>& fset, vector<double>& freq_current) {
  Int N = rank.size();
  double freq = 0.0;
...
double kl(vector<double>& freq_current, vector<Int>& cl, double freq, Int N0, Int N) {
  double r0 = (double)N0 / (double)N;
  double r1 = (double)(N - N0) / (double)N;
  double freq0 = 0.0;
  for (Int i = 0; i < N; ++i) {
    if (cl[i] == 0) freq0 += freq_current[i];
  }
  freq0 /= (double)N;
...
void runFPM(vector<vector<double>>& rankn, vector<Int>& fset, vector<double>& freq_current, Int i_prev, Int n, Int size_limit, ofstream& ofs) {
  ...
  for (Int i = i_prev + 1; i < n; i++) {
    ...
    if (freq > SIGMA && fset.size() <= size_limit) {
      if (VERBOSE) cout << "  Frequency of {" << fset << "} = " << freq << endl;
      // ofs << fset << " (" << freq << ")" << endl;
      NUM_PATTERN += 1.0;
...
void runSPM(vector<vector<double>>& rankn, vector<Int>& fset, vector<double>& freq_current, Int i_prev, Int n, Int size_limit, ofstream& ofs, Int N0, set<pair<double, double>, cmp>& freq_pval_list, double alpha) {
  ...
  for (Int i = i_prev + 1; i < n; i++) {
	...
	ADMISSIBLE = freq_pval_list.empty() ? 1e20 : alpha / (*prev(freq_pval_list.end())).second;
      ...
      // }
      // ofs << fset << " (" << freq << ")" << endl;
      if (VERBOSE) cout << "  Current freq threshold =    " << SIGMA << endl;
...
void runSPMWY(vector<vector<double>>& rankn, vector<Int>& fset, vector<double>& freq_current, Int i_prev, Int n, Int size_limit, ofstream& ofs, Int N0, set<pair<double, double>, cmp>& freq_pval_list, vector<vector<Int>>& cl_perm, vector<double>& pmin_perm, double alpha) {
  Int N = rankn.size();
  for (Int i = i_prev + 1; i < n; i++) {
    fset.push_back(i);
    double freq = computeFreqUpdate(rankn, i, freq_current);
	...
	double p_tmp = computePvalue(kl(freq_current, cl_perm[ip], freq, N0, N), N);
	if (p_tmp < pmin_perm[ip]) pmin_perm[ip] = p_tmp;
      }
      Int counter = 0;
      for (auto&& x : pmin_perm) {
	if (x < (*prev(freq_pval_list.end())).second) counter++;
      }
	...
	if (freq_pval_list.empty()) {
      ...
      if (VERBOSE) cout << "  Current freq threshold =    " << SIGMA << endl;
      if (VERBOSE) cout << "  Current admissible number = " << ADMISSIBLE << endl;
      runSPMWY(rankn, fset, freq_current, i, n, size_limit, ofs, N0, freq_pval_list, cl_perm, pmin_perm, alpha);
    }
    // extract the feature fset.back();
    for (Int i = 0; i < N; ++i) {
      freq_current[i] /= (double)rankn[i][fset.back()];
...
int main(int argc, char *argv[]) {
  ...
  while ((opt = getopt(argc, argv, "i:c:o:t:s:a:k:fvwp:b")) != -1) {
    ...
    case 'w': WY = true; break;
    case 'p': wy_repeat = atoi(optarg); break;
  ...
  if (VERBOSE) cout << "  Frequency of (" << fset << ") = " << computeFreq(rank, fset, freq_current) << endl;
  ...
  if (!WY && !bonferroni) {
    ...
    for (auto&& cl_each : cl_perm) {
      ...
      shuffle(cl_each.begin(), cl_each.end(), default_random_engine(seed));
      seed++;
    }
    vector<double> pmin_perm(cl_perm.size(), 1.0);
    runSPMWY(rankn, fset, freq_current, -1, n, size_limit, ofs, N0, freq_pval_list, cl_perm, pmin_perm, alpha);
    sort(pmin_perm.begin(), pmin_perm.end());
    // cout << pmin_perm << endl;
  ...
  // ------------------------------------------------------------------------- //
  cout << "> Run Frequent Pattern Mining with a threshold " << SIGMA << endl << flush;
